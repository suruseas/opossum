	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !force {
				...
				if !ok {
					// A decline never reaches the after-the-fact leftovers report,
					// so this is the one line that keeps them discoverable (#353
					// puts the list after the decision; "no" means it never comes).
					fmt.Fprintln(out, "Left everything as it was. `opossum destroy --dry-run` shows the full picture, kept items included.")
					return nil
				}
			}
...
func adaptProject(stderr io.Writer, o *orchestrator.Orchestrator, dryRun bool, named, profiles []string) (*orchestrator.Orchestrator, error) {
	if len(composeFiles) != 0 {
		...
		body, changes, gated := o.PlanOverlayFor(named)
		reportGated(stderr, gated)
		switch {
	...
	if existing := compose.DiscoverOpossumOverlay(o.Project.BaseDir); existing != "" {
		if len(changes) > 0 {
			...
			reportEntries(stderr, changes)
			// The overlay is written once, and a suggestion driven by a crash arrives
			// after that: the service had to start and fail first. So this is the path
			// those land on, and the advice has to be right about whose file it is.
			// Deleting one opossum wrote costs nothing — it is regenerated from the
			// same detection. Deleting a hand-written one throws away someone's work.
			if orchestrator.GeneratedOverlay(existing) {
				...
				fmt.Fprintf(stderr, "opossum: %s is your own file (no opossum header), so it is not opossum's to replace. "+
					"To see the blocks it would have written, move yours aside (`mv %[1]s %[1]s.mine`) and re-run — "+
					"then copy what you want back in.\n", filepath.Base(existing))
			}
		}
		return nil, nil
	}
...
// reportGated names the services the plan did not look at because their
// profile is not active in this run. Said once, up front, so an entry that is
// missing is missing for a reason the reader was told — and said as a fact
// about this run, not as an errand: a later run with the profile enabled
// finds the overlay already in place and reports what those services would
// need.
func reportGated(stderr io.Writer, gated []string) {
	if len(gated) == 0 {
		return
	}
	fmt.Fprintf(stderr, "opossum: %d service(s) gated behind a profile not enabled in this run were not "+
		"looked at (docker compose leaves them out of `config` the same way): %s. A run with the "+
		"profile enabled reports what they would need.\n", len(gated), strings.Join(gated, ", "))
}
