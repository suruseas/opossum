...
- opossum runs a **subset** of the compose schema. A field docker compose takes
  that opossum does not act on is ignored with a warning (not a hard error) so a
  `docker-compose.yml` loads without surprises; a key docker compose does not take
  at all (a typo such as `enviroment:`) is refused before anything runs, as docker
  compose refuses it. `opossum config` prints the resolved project + the ignored fields.
...
opossum ps                                   # SERVICE / CONTAINER / IMAGE / IP / PORTS / STATUS
opossum logs --follow web                    # stream a service's logs (no -f: that is the global --file)
...
- The `OPSM-105` suggestion for an app's own data directory is written only for a
  mount that has actually died chowning: when a container exits with `chown …
  Operation not permitted` on a bind mount and opossum can tell which mount it was,
  `up` records the service, container path and host path in
  `.opossum/chown-failures.json`, and the next `up --from-docker-compose` turns each
  record into a suggestion naming that mount and no other (a record whose service or
  mount is gone, or whose mount is already a named volume, is skipped). `destroy`
  removes the file with the rest of `.opossum/`.
...
| `destroy` | remove **everything opossum created for this project** and stop there: containers (including orphans and `run` leftovers), the project network and any it declared, named/anonymous volumes, images built or pulled for it, the restart supervisor, the `.opossum/` state directory and the generated `compose.opossum.yaml` (**an overlay opossum did not generate is kept** — it carries no `# Generated by …` header, so it is yours; destroy says it kept it. One opossum generated that you then edited still has the header, so it **is** removed — pass `--keep-overlay` to keep it). **Never touches the compose file, `.env` or sources**, and never anything shared: volumes declared `external: true`, other projects' containers, the DNS domain and the builder cache all stay (the last two are printed with the command to remove them). **`ws` snapshots are never removed** — a `.opossum-snapshots/` directory belongs to the directory that was snapshotted, not to a project — but any found in the project's directory or one level below it are printed with the path, since they are usually the largest thing left on disk. Asks first; `--force` skips the question (no `-f` shorthand — that is the global `--file`) (**required** when stdin isn't a terminal — it errors rather than guess), `--dry-run` lists and stops (and does **not** start the runtime), `--keep-overlay` keeps a generated `compose.opossum.yaml`, `--keep-images` keeps the images (a pulled one may be shared with other projects). `--keep-local` leaves this directory's generated files alone and removes only the runtime objects — what you want when destroying a project by name from elsewhere; **`-p <other>` with `--force` is refused** when it would take this directory's files with it, since the runtime objects belong to the name and the files belong to the directory. Volumes named for the project that no service claims (left by a renamed or deleted service) are **listed, not removed** — opossum cannot tell one from an `external: true` volume by name alone. Containers are removed only when their `opossum.project` label matches, so a namesake is safe. After removing, it checks again and **errors naming whatever survived** — a delete can exit 0 and change nothing. Needs the compose file to enumerate the project; if it is gone, `down` still stops a stranded supervisor |
| `ps` | table of service / container / image / IP / ports / status. STATUS is `running` or `stopped`; a service whose container does not exist gets **no row** (there is no `absent` value), and so does one whose container of the name is another project's, carries no `opossum.project` label (made outside opossum), or that the runtime gives no readable answer about — that one is named on stderr, and the third kind makes the exit non-zero once the table is printed. It does **not** show healthcheck state; to confirm a service is *healthy*, check its `logs` (a `service_healthy` dependency gates on health automatically during `up`). `--format json` prints `[{"Service":…,"Container":…,"Image":…,"IP":…,"Ports":…,"Status":…}]` in the same order, with `""` where the table shows `-` (`Ports` is the table's string, not a list) |
| `port <service> <container-port>` | prints the host side of one published port on a single line, `0.0.0.0:65345` — the way to read the host port when `ports: - "3000"` was moved off a busy 3000 (`HOST=$(opossum port web 3000)`); `--protocol tcp\|udp` (tcp by default). A port that is not published exits 1 listing the ones that are (`no port 90/tcp for container …: 80/tcp, 90/udp`); an absent or stopped container exits 1 with `service "web" is not running` (a container of the name that is another project's, carries no `opossum.project` label, or that the runtime gives no readable answer about, running or not, exits 1 with `service "web" has no container of this project's: …` instead). Read-only: never starts the runtime |
...
| `logs [service…]` | print logs, each line prefixed with its service as docker compose writes it off a terminal (`web-1  \| …`, no colour), one service or several, followed or not; Ctrl-C or SIGTERM exits 130 quietly; `--no-log-prefix` prints the lines as the container wrote them; `--follow` streams (several services multiplexed) — no `-f` shorthand, that is the global `--file`; `-n/--tail N`; a container of a service's name that is another project's, carries no `opossum.project` label, or that the runtime gives no readable answer about is not read and is named on stderr; with every container asked for someone else's, nothing is printed and the exit is zero; one the runtime gives no readable answer about makes the exit non-zero once the rest are read |
| `stats [service…]` | live CPU/mem/net/IO (streams); `--no-stream` for one snapshot; `--host` shows each service's host-memory footprint (its VM's resident size — a shared-VM tool can't do this per service); `--format json` (requires `--no-stream`; not with `--host`) prints `[{"Service":…,"Container":…,"CPUUsageUsec":…,"MemoryUsageBytes":…,"MemoryLimitBytes":…,"NetworkRxBytes":…,"NetworkTxBytes":…,"BlockReadBytes":…,"BlockWriteBytes":…,"NumProcesses":…}]` in service order, with no row for a service whose container is stopped or absent (`[]` when none of those that have a container is running; an error when none has a container); a container of a service's name that is another project's, carries no `opossum.project` label, or that the runtime gives no readable answer about is not measured and is named on stderr (with `--host`, it has no row); with every container asked for someone else's, nothing is printed and the exit is zero; one the runtime gives no readable answer about makes the exit non-zero once the rest are measured |
| `exec [-it] <service> <cmd…>` | run a command in a running container; `-i`/`--interactive` keeps stdin open, `-t`/`--tty` allocates a TTY; exits with the command's exit code; refuses a container of the same name that another project owns, or that carries no `opossum.project` label (made outside opossum) |
...
| `cp <src> <dst>` | copy files host↔container; each path is a host path or `service:path`; refuses a container of the same name that another project owns, or that carries no `opossum.project` label (made outside opossum) |
| `start [service…]` / `stop [service…]` / `restart [service…]` | start / stop / stop-then-start existing containers; a container of the same name that another project owns, or that carries no `opossum.project` label (made outside opossum), is left, and said so |
| `kill [service…]` | send a signal (default KILL); `-s/--signal <SIG>`; a killed service is not brought back by the restart supervisor, as docker compose, until `up`/`start`/`restart` (after a kill the container outlived, docker brings supervision back with `restart` only); a kill container 1.4.1 fails, when the container is then found running (`-s BOGUS`) or the runtime gives no readable answer about it, exits 1 and leaves the service's record of a stop as it was; a container of the same name that another project owns, or that carries no `opossum.project` label, is left, and said so |
...
| `doctor` | diagnose the environment (runtime, DNS domain, outbound network, builder memory, reclaimable storage, networks nothing is running on, stack-memory estimate); non-zero exit if any check fails. `--format json` for a machine-readable report |
...
**Ignored (file still loads):** `container_name`, `dns`, `dns_search`,
`network_mode` values other than `none` (e.g. `host` → the service joins the
project network), a service's per-network `aliases` and static IPs
(`ipv4_address`), the keys of a top-level network's `ipam` other than a `config`
entry's `subnet` (the subnet is given to `network create --subnet` /
`--subnet-v6`),
...
(`[OPSM-501]`/`[OPSM-502]`). Per-network settings are named individually —
`networks.<net>.ipam.driver` for a top-level declaration, `networks.<net>.aliases` /
`networks.<net>.ipv4_address` under a service — the same way per-volume
fields are.
...
- **`[OPSM-204]` … `mounts the Docker socket, which does not answer for the
  ...
  Docker daemon was reached that way from inside a container on 2026-08-28.
  That is the one that has been measured; what a session socket mount reaches
  here has not been (`OPSM-106`). What puts a symlink at that name varies by
  which Docker distribution you have, and one merely installed answers nothing, so the
...
- **`[OPSM-207]` … `network <n> exists with IPv4 subnet <a>, and the compose file now declares <b>`** →
  ...
  keeps a network while the project is up rather than recreating it under running
  containers (docker compose recreates it). Run `opossum down` (which removes the
  network) and `up` again to get the new subnet, or remove `ipam` to keep the
  existing network.
- **`[OPSM-208]` … `another opossum command is changing project <p> (pid N)`** → an
  `up`, `down` or `destroy` for the same project is still running in another terminal.
  Wait for it to finish, then retry. Two at once used to break a project (the
  first's rollback removed a service the second had just reported as up to
  date), so the second is refused at once rather than overlapped. The lock is
  released when the other command exits, however it exits — nothing to clean. A
  foreground `up` (`--foreground`) holds it until the service exits or Ctrl-C, so
  waiting alone does not free it: end that `up` first.
...
- **`[OPSM-107]` … mounts a path that names a file, so a directory was created** → a bind
  ...
  thing that can be created, and docker compose does the same. When the mount names a
  file (`./init.js:/docker-entrypoint-initdb.d/init.js`) the service then finds a
...
- **`[OPSM-408]` … `watching <services> for restart:`** → the project declares
  ...
  it logs every restart to `supervisor.log` under `$XDG_STATE_HOME/opossum/<project>/`
  (next to `supervisor.pid`; `$XDG_STATE_HOME` defaults to `~/.local/state` — outside
  the project, so a repository never grows pid files). Skip it with `up --no-supervisor` or
  `OPOSSUM_NO_SUPERVISOR=1` (useful in CI, where a background watcher outliving the
  job is worse than a service staying down).
- **`[OPSM-409]` … `restarted <service>` / `giving up on <service>`** → the
...
- `OPSM-409` — the supervisor restarted a service, gave up on one, or left a container that is another project's or carries no project label.
...
- `OPSM-603` — a `watch` file sync failed, or was skipped because the container belongs to another project, carries no project label, or the runtime would not say whose it is.
