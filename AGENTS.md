...
| `config [--services]` | validate and print the resolved compose (interpolation + env_file applied), listing ignored fields; `--services` prints names only, resolving no `env_file` (interpolation still runs) |
...
- **`[OPSM-408]` … `watching <services> for restart:`** → the project declares
  ...
  it logs every restart. Skip it with `up --no-supervisor` or
