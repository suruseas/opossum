...
// FaviconAssets are copied into the site verbatim from docs/assets. The browser
// tab is the one place the site is visible before anyone reads a word, and the
// primer theme ships no icon of its own — without these every tab shows the
// browser's default globe.
var FaviconAssets = []string{"favicon.png", "favicon-512.png", "apple-touch-icon.png"}
...
func Build(root, out string) error {
	...
	if err := os.MkdirAll(filepath.Join(out, "_includes"), 0o755); err != nil {
		return err
	}
	if err := os.WriteFile(filepath.Join(out, "_includes", "head-custom.html"), []byte(headCustomHTML), 0o644); err != nil {
		return err
	}
	if err := os.MkdirAll(filepath.Join(out, "assets"), 0o755); err != nil {
		return err
	}
	for _, name := range FaviconAssets {
		icon, err := os.ReadFile(filepath.Join(root, "docs", "assets", name))
		if err != nil {
			return err
		}
