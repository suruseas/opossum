...
var allDiagCodes = []diagCode{
	...
	codeIgnoredTopField, codeIgnoredField, codeRuntimeAbsent, codeRuntimeStopped,
...
func ErrRuntimeStopped() error {
	return fmt.Errorf("[%s] the `container` system isn't running — start it with "+
		"`container system start` (run `opossum doctor` to check the environment)", codeRuntimeStopped)
}
