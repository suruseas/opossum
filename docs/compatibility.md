...
| `env_file` | ✅ | string or list (short, or long `{path, required}`); `KEY=VALUE` files folded in, `environment` overrides them. A missing file — or one that fails to expand — errors unless `required: false`, and only where the environment is needed: `config`, `up` and `run` raise it, while `ps`, `logs` and `config --services` do not. A service that `profiles:` keeps out of this run is not read by any of them. Known difference: an env file listed twice (in one file, or across `-f` files) is read each time here, so its values stand where they last appear; docker compose keeps only its first listing |
...
the environment all start it again, and it ends with `[OPSM-407]` again. The
last of those needs no flag at all, so a profile is no defence where something
else sets it.
