...
- **Shape checks at load**: a file is checked the way docker compose validates one before anything runs, and refused with the field, the line and what to write instead — a field written with nothing after it (`volumes:` alone; `command:` and `entrypoint:` alone still mean "no command"), a number or a boolean where a string belongs (`image: 42`, `volumes: [42]`, `profiles: [42]`), a bare name where only a list is taken (`cap_add: NET_ADMIN`), an empty item (`- ` alone), a network listed twice, a service key with nothing under it, the same shapes one level down (`build.context`, `deploy.resources` limits and reservations, `healthcheck`, `develop.watch` rules), a watch rule's `action` that is not one docker compose takes or that copies files without a `target`, and a blank `healthcheck` duration (`interval: ""`). YAML aliases (`*name`) are read wherever a value or a list item can stand. Known differences from docker compose: with several `-f` files the checks run on the merged document, so a key a later file writes with nothing after it keeps the earlier file's value and a file's own mistakes are not reported per file; `healthcheck.retries: 0` means the default rather than zero; a watch rule's `action` may be left out; and a key a watch rule does not have is listed among the ignored fields rather than refused.
