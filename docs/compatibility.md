...
| `start [service…]` | ✅ | start existing (stopped) containers, in dependency order; a service that fails to start does not stop the others, but a service that depends on it is not started, as in docker compose. A service with no container is an error (exit 1, pointing at `opossum up`), and its dependents are not started either — docker compose skips it silently, starts its dependents and exits 0. Named services are started without their dependencies (docker compose starts those first) |
| `stop [service…]` | ✅ | stop without removing |
| `restart [service…]` | ✅ | stop then start in place; every service stopped is started again even if one before it fails (dependencies are not consulted, as in docker compose). A service with no container is an error (exit 1) — docker compose skips it silently and exits 0 |
