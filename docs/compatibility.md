...
| `ps [--format json]` | ✅ | service / container / image / IP / ports / status; `--format json` prints the same columns as an array of objects keyed `Service`, `Container`, `Image`, `IP`, `Ports`, `Status` — an empty string where the table shows `-`, and `Ports` as the table's string (not the shape `docker compose ps --format json` prints) |
