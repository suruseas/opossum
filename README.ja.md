...
| `ports` | ✅ | `container run -p` に渡す。短形式（`"8080:80"`、`"3000"`）と長形式（`{target, published, protocol, host_ip}`）の両対応。コンテナポートのみの指定にはホストポートを補う（Apple のランタイムでは必須のため）。空いていれば同番、埋まっていれば空きポートを使い、その旨を通知する |
...
このファイルは「直したもの」だけでなく、**そのプロジェクトの互換性の全体像**です。直せなかったものも同じ場所に載るので、別の場所を知らないと気づけない、という状態になりません。エントリは3種類あり、それぞれ明示されます：

- **applied** — 変更済みで、効いています。これがプロジェクトを動かしているものです。
- **suggestion — NOT APPLIED** — 具体的な変更を書き出しますが**コメントアウト**されています。プロジェクトの意味（データの置き場所・サービス間の共有の仕方）を変えるためです。ブロックごとコメントを外せば適用でき、必要な `volumes:` 宣言も同梱されているので単体で完結します。
- **note** — 変えるものがありません。compose ファイルでは修正を表現できないもの（Docker ソケットのマウント・ホストのデバイス）です。失敗が原因不明にならないよう記録します。YAML を持たないので、外すべきコメントもありません。

各エントリには、何についてか・なぜか（診断コード付き）が必ず書かれ、applied には確認方法と戻し方、suggestion には適用方法と無視の仕方、note には代わりに何を期待すべきかが続きます。**既存の `compose.opossum.yaml` は決して上書きしません**。あなたの compose ファイルも変更しません。
...
| 公開ポート | `ports: - "3000"` はランダムなホストポート | 空いていれば `3000:3000`、埋まっていれば空きポートへ退避して通知（実際のポートは `opossum ps` で確認。ホスト側を書かないサービスが複数あれば互いに別のポートになる）。Apple `container` はホストポートの明示が必須でランダム割り当てがないため、同番はランダムの代わりの「予測しやすい既定値」 |
